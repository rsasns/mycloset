...
@section('content')
    ...
    <div class="container">
        ...
        <nav class="nav nav-pills nav-fill mb-4">
            <a href="{{ route('users.followers', ['user_id' => $user->user_id]) }}" class="nav-item nav-link active btn-color link disabled {{ Request::routeIs('users.followers') ? 'active' : '' }}">
            フォロワー
            </a>
            <a href="{{ route('users.followings', ['user_id' => $user->user_id]) }}" class="nav-item nav-link text-dark link {{ Request::routeIs('users.followings') ? 'active' : '' }}">
            フォロー中
            </a>
        </nav>
        @include('users.users')
