@if(Request::routeIs('cordinates.show'))
        @if (Auth::user()->is_nice($cordinates->id))
            {{-- いいねを外す --}}
            <span>
                {!! Form::open(['route' => ['user.unnice', $cordinates->id], 'method' => 'delete', 'class' => 'form-inline']) !!}
                    {!! Form::button('<i class="fas fa-heart"></i>', ['class' => "btn rounded-pill btn-color", 'type' => 'submit']) !!}
                    <small class="ml-1">{{ $favorites_users->nice_users_count }}スキ！</small>
                {!! Form::close() !!}
            </span>
        @else
            {{-- いいねをする --}}
            <span>
                {!! Form::open(['route' => ['user.onnice', $cordinates->id]]) !!}
